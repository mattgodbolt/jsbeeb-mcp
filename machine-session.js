...
// Derive the jsbeeb package root so we can chdir to it during initialise().
// jsbeeb loads ROMs via relative paths (e.g. "roms/os.rom") so cwd must be
// the jsbeeb root when initialise() runs.
const _jsbeebRoot = path.dirname(
    path.dirname(fileURLToPath(import.meta.resolve("jsbeeb/src/fdc.js"))),
);
...
export class MachineSession {
    ...
    async initialise() {
        // jsbeeb loads ROMs via paths relative to cwd (e.g. "roms/os.rom").
        // Set cwd to the jsbeeb package root once — this is a server process
        // so there's no need to restore the original working directory.
